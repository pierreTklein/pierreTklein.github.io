...
function animations() {
    ...
    function arrow(mass, x, y, dx, dy) {
        ...
        this.mass = mass;
        this.radius = Math.sqrt(8 / Math.PI);

        this.maxV = 2.5;
        this.velocity = new vector(dx, dy);
        this.velocity.limit(this.maxV);
        this.heading = 1; //this.velocity.getAngle();
        ...
        this.stayInBound = function (context) {
            this.location.x = (this.location.x < 0) ? canvas.width / dpr : this.location.x;
            this.location.y = (this.location.y < 0) ? canvas.height / dpr : this.location.y;
            this.location.x = (this.location.x > canvas.width / dpr) ? 0 : this.location.x;
            this.location.y = (this.location.y > canvas.height / dpr) ? 0 : this.location.y;
        }
    ...
    function getArrows(numArrows) {
        ...
        for (var i = 0; i < numArrows; i++) {
            ...
            arr.push(new arrow(1, x / dpr, y / dpr, dx, dy))
        }
        return arr;
    }
    ...
    function draw() {
        ...
        context.clearRect(0, 0, canvas.width * dpr, canvas.height * dpr); // clear canvas
        if (frameRate < 20 && !optimizedSize) {
            optimizedSize = true;
